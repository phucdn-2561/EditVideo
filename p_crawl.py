...
def crawl_chapter_text(url):
    """Crawls text content from the specified HTML structure, handling potential issues."""
    try:
        ...
        if chapter_div:
            p_tags = chapter_div.find_all('p')
            chapter_text = [p.get_text(strip=True) for p in p_tags if p.get_text(strip=True)]
            return '\n'.join(chapter_text)
        else:
            return None
...
if __name__ == "__main__":
    base_url = "https://truyenfull.vision/vinh-hang-thanh-vuong/"
